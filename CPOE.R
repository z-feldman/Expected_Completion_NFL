...
positions <- read_csv("https://raw.githubusercontent.com/z-feldman/nfl_data/master/rosters.csv")
positions %<>% mutate(
...
plays <- left_join(x = plays,
                   y = positions %>% select(season, name, team, receiver_position = position),
                   by = c("season", "receiver_player_name" = "name", "posteam" = "team"))
plays <- left_join(x = plays,
                   y = positions %>% select(season, name, team, passer_position = position),
                   by = c("season", "passer_player_name" = "name", "posteam" = "team"))
...
pred4 <- predict.bam(model4, newdata = test, se.fit = TRUE, type = "link")


plot_pred4 <- data.frame("complete_pass" = test$complete_pass, "logit" = pred4$fit, "se" = pred4$se.fit)
plot_pred4 %<>% mutate(prob = plogis(logit),
                       high_prob = plogis(logit + se*3),
                       low_prob = plogis(logit- se*3))
...
summary_ben
gratia::draw(ben_ec)
pred_ben <- predict.bam(ben_ec, newdata = test, se.fit = TRUE, type = "link")


plot_ben <- data.frame("complete_pass" = test$complete_pass, "logit" = pred_ben$fit, "se" = pred_ben$se.fit)
plot_ben %<>% mutate(prob = plogis(logit),
                       high_prob = plogis(logit + se*2),
                       low_prob = plogis(logit- se*2))
...
pred_air <- predict.bam(air_ec, newdata = test, se.fit = TRUE, type = "link")


#  * Air Yards Predictions df and Investigating 


plot_air <- data.frame("complete_pass" = test$complete_pass, "logit" = pred_air$fit, "se" = pred_air$se.fit)
plot_air %<>% mutate(prob = plogis(logit),
                     high_prob = plogis(logit + se*2),
                     low_prob = plogis(logit- se*2))
